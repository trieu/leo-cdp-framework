...
public final class ObserverHttpGetHandler {
	...
	public final static boolean process(RoutingContext context, HttpServerRequest req, String urlPath, MultiMap reqHeaders, MultiMap params,
		...
		// SHORT LINK CLICK, redirect for O2O synchronization E.g: https://demotrack.leocdp.net/ct/294HGH6ZYaYyOBqdTnjhG3
		...
		// feedback score
			...
			int previousFeedbackScore = FeedbackDataManagement.loadLatestFeedbackScore(templateId, refVisitorId, srcTouchpointUrl);
		...
		//
			...
			List<TargetMediaUnit> items = ProfileGraphManagement.queryRecommendedContents(observerId, visid, touchpointUrl, startIndex, numberResult);
			Gson gson = new GsonBuilder().excludeFieldsWithoutExposeAnnotation().create();
			resp.end(gson.toJson(items));
			return true;
		}
