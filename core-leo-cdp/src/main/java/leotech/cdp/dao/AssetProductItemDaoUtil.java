...
public class AssetProductItemDaoUtil extends AssetItemDao {
	...
	static final String AQL_GET_PRODUCT_ITEM_BY_PRODUCT_ID_AND_ID_TYPE = AqlTemplate.get("AQL_GET_PRODUCT_ITEM_BY_PRODUCT_ID_AND_ID_TYPE");
	...
	public static ProductItem getById(String id, boolean loadCategoryData) {
		if(StringUtil.isNotEmpty(id)) {
			...
			bindVars.put("id", id);
			ProductItem p = new ArangoDbCommand<ProductItem>(db, AQL_GET_PRODUCT_ITEM_BY_ID, bindVars, ProductItem.class, new CallbackQuery<ProductItem>() {
				@Override
	...
	public static ProductItem getByUrl(String url) {
		...
		ProductItem p = new ArangoDbCommand<ProductItem>(db, AQL_GET_PRODUCT_ITEM_BY_FULL_URL, bindVars, ProductItem.class).getSingleResult();
		return p;
	}
	...
	public static ProductItem getByProductId(String productId, String productIdType) {
		...
		ProductItem p = new ArangoDbCommand<ProductItem>(db, AQL_GET_PRODUCT_ITEM_BY_PRODUCT_ID_AND_ID_TYPE, bindVars, ProductItem.class).getSingleResult();
		return p;
	}
	...
	public static List<ProductItem> list(int startIndex, int numberResult) {
		...
		List<ProductItem> list = new ArangoDbCommand<ProductItem>(db, AQL_GET_PRODUCT_ITEMS, bindVars,ProductItem.class).getResultsAsList();
		return list;
	}
	...
	public static Map<String, String> searchProductItemsByKeywords(String keywords, int startIndex, int numberResult) {
		...
		bindVars.put("startIndex", startIndex);
		bindVars.put("numberResult", numberResult);
		List<ProductItem> list = new ArangoDbCommand<ProductItem>(db, AQL_SEARCH_PRODUCT_ITEMS_BY_KEYWORDS, bindVars,ProductItem.class).getResultsAsList();
	...
	public static List<ProductItem> getByCampaign(String campaignId) {
		...
		List<ProductItem> list = new ArangoDbCommand<ProductItem>(db, AQL_GET_PRODUCT_ITEMS_BY_CAMPAIGN, bindVars, ProductItem.class).getResultsAsList();
		return list;
	}
	...
	public static List<ProductItem> list(String searchValue, String groupId, int startIndex, int numberResult) {
		...
		List<ProductItem> list = new ArangoDbCommand<ProductItem>(db, AQL_GET_PRODUCT_ITEMS_IN_GROUP, bindVars, ProductItem.class).getResultsAsList();
		return list;
	}
	...
	public static int countTotalSizeOfGroup(String searchValue, String groupId) {
		...
		Integer count = new ArangoDbCommand<Integer>(db, AQL_COUNT_PRODUCT_ITEMS_IN_GROUP, bindVars, Integer.class).getSingleResult();
		return count;
	}
