...
public final class SegmentDaoUtil extends AbstractCdpDatabaseUtil {
	...
	static final String AQL_COUNT_TOTAL_ACTIVE_SEGMENTS = "RETURN LENGTH(FOR s in " + Segment.COLLECTION_NAME
			+ " FILTER s.status >= 0 RETURN s._key)";
	...
	static final String AQL_GET_ALL_SEGMENTS_BY_AUTHORIZATION = AqlTemplate
			.get("AQL_GET_ALL_SEGMENTS_BY_AUTHORIZATION");
	static final String AQL_GET_ALL_SEGMENT_REFS_BY_AUTHORIZATION = AqlTemplate
			.get("AQL_GET_ALL_SEGMENT_REFS_BY_AUTHORIZATION");
	...
	static final String AQL_REMOVE_VIEWABLE_SEGMENTS_FOR_USER = AqlTemplate
			.get("AQL_REMOVE_VIEWABLE_SEGMENTS_FOR_USER");
	static final String AQL_REMOVE_EDITABLE_SEGMENTS_FOR_USER = AqlTemplate
			.get("AQL_REMOVE_EDITABLE_SEGMENTS_FOR_USER");
	...
	public static List<Segment> getAllSegmentsByAuthorization(SegmentFilter filter) {
		...
		bindVars.put("authorizedViewer", filter.getAuthorizedViewer());
		bindVars.put("authorizedEditor", filter.getAuthorizedEditor());
		ArangoDbCommand<Segment> q = new ArangoDbCommand<Segment>(db, AQL_GET_ALL_SEGMENTS_BY_AUTHORIZATION, bindVars,
				Segment.class);
		List<Segment> list = q.getResultsAsList();
		return list;
	}
	...
	public static List<SegmentRef> getAllSegmentRefs(SegmentFilter filter) {
		...
		bindVars.put("authorizedViewer", filter.getAuthorizedViewer());
		bindVars.put("authorizedEditor", filter.getAuthorizedEditor());
		ArangoDbCommand<SegmentRef> q = new ArangoDbCommand<SegmentRef>(db, AQL_GET_ALL_SEGMENT_REFS_BY_AUTHORIZATION,
				bindVars, SegmentRef.class);
	...
	public static Set<RefKey> getRefKeysByIds(List<String> idList) {
		...
		Set<RefKey> set = new ArangoDbCommand<>(db, AQL_GET_SEGMENT_REFKEYS_BY_IDS, bindVars, RefKey.class)
				.getResultsAsSet();
		return set;
	}
	...
	public static Segment getSegmentById(String id, boolean fullDataLoad) {
		...
		Map<String, Object> bindVars = new HashMap<>(1);
		bindVars.put("id", id);
		Segment segment = new ArangoDbCommand<Segment>(db, AQL_GET_SEGMENT_BY_ID, bindVars, Segment.class)
				.getSingleResult();
		if (segment != null && fullDataLoad) {
			...
			activationRules.forEach(rule -> {
				...
				String agentId = rule.getAgentId();
				if (StringUtil.isNotEmpty(agentId)) {
					Agent c = AgentDaoUtil.getById(agentId);
					if (c != null) {
						rule.setAgentName(c.getName());
					}
				}
			});
	...
	public static List<Segment> loadSegmentsToRefresh(int startIndex, int numberResult) {
		...
		List<Segment> list = new ArangoDbCommand<Segment>(db, AQL_GET_SEGMENTS_TO_REFRESH, bindVars, Segment.class)
				.getResultsAsList();
		return list;
	}
	...
	public static List<Segment> getAllSegmentsForRecommender() {
		ArangoDatabase db = getCdpDatabase();
		List<Segment> list = new ArangoDbCommand<Segment>(db, AQL_GET_SEGMENTS_FOR_RECOMMENDER, Segment.class)
				.getResultsAsList();
		return list;
	}
	...
	public static List<Segment> getAllActiveSegments() {
		ArangoDatabase db = getCdpDatabase();
		List<Segment> list = new ArangoDbCommand<Segment>(db, AQL_GET_ALL_ACTIVE_SEGMENTS, Segment.class)
				.getResultsAsList();
		return list;
	}
	...
	public static List<Segment> getAllRemovedSegmentsToDeleteForever() {
		ArangoDatabase db = getCdpDatabase();
		List<Segment> list = new ArangoDbCommand<Segment>(db, AQL_GET_SEGMENTS_TO_DELETE_FOREVER, Segment.class)
				.getResultsAsList();
		return list;
	}
	...
	public static int getMaxIndexScoreInSegments() {
		ArangoDatabase db = getCdpDatabase();
		Integer c = new ArangoDbCommand<Integer>(db, AQL_GET_MAX_INDEX_SCORE_IN_SEGMENTS, Integer.class)
				.getSingleResult();
