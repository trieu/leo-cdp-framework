...
public final class TouchpointHubManagement {
	...
	static final LoadingCache<String, TouchpointHub> cacheById = CacheBuilder.newBuilder().maximumSize(50000)
			.expireAfterWrite(5, TimeUnit.MINUTES).build(cacheLoaderById);
	static final LoadingCache<String, TouchpointHub> cacheByObserverId = CacheBuilder.newBuilder().maximumSize(50000)
			.expireAfterWrite(QUERY_CACHE_TIME, TimeUnit.HOURS).build(cacheLoaderByObserverId);
	static final LoadingCache<String, List<TouchpointHub>> cacheByJourneyId = CacheBuilder.newBuilder()
			.maximumSize(50000).expireAfterWrite(1, TimeUnit.MINUTES).build(cacheLoaderByJourneyId);
	...
	/**
	 * @param observerId if observerId not found, the DATA_OBSERVER is return
	 ...
	 */
	public static List<TouchpointHubReport> getTouchpointHubReport(String journeyId, String beginFilterDate,
			String endFilterDate, int maxTouchpointHubSize) {
		return TouchpointHubDaoUtil.getTouchpointHubReport(journeyId, beginFilterDate, endFilterDate, 0,
				maxTouchpointHubSize);
	}
