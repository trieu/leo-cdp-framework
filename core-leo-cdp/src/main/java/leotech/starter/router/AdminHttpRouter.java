...
/**
 * Admin Http Web Router Refactored for Leo CDP (Java 11) * @author tantrieuf31
 * 
 * @since 2020
 */
public final class AdminHttpRouter extends BaseWebRouter {
    ...
    /**
     * Helper to invoke handler methods via Reflection if a common interface is
     * missing. Note: Ideally, all your Handlers should implement a common interface
     * (e.g. AdminRequestHandler) so you can cast 'handler' and call methods
     * directly without reflection.
     */
    @SuppressWarnings("unchecked")
    private JsonDataPayload invokeHandlerMethod(Object handler, String methodName, Object[] args,
            Class<?>[] paramTypes) {
        try {
            var method = handler.getClass().getMethod(methodName, paramTypes);
            return (JsonDataPayload) method.invoke(handler, args);
        } catch (Exception e) {
            throw new RuntimeException("Failed to invoke " + methodName + " on " + handler.getClass().getSimpleName(),
                    e);
        }
    }
}