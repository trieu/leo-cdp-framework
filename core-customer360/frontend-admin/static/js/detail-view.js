...
(function (C360) {
  ...
  function buildDetailVm(profile, engagement, channelActivity, topInterests, timeline) {
    ...
    if (profile.email) channels.push({ icon: "✉️", label: "Email", badge: profile.email });
    if (profile.phone_number) channels.push({ icon: "☎️", label: "Phone", badge: profile.phone_number });
