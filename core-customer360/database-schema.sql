...
CREATE TABLE IF NOT EXISTS customer360.cdp_profile_attributes (
    ...
    attribute_group VARCHAR(50) NOT NULL DEFAULT 'GENERAL' CHECK (attribute_group IN (
        ...
        'LINEAGE', 'LIFECYCLE', 'LEAD_SCORING', 'CHURN_SCORING', 'CLV_SCORING', 'CX_SCORING',
...
CREATE INDEX idx_crm_transactions_tenant_time ON customer360.crm_transactions (tenant_id, transaction_time DESC);
CREATE INDEX idx_crm_transactions_master_profile ON customer360.crm_transactions (master_profile_id, transaction_time DESC) WHERE master_profile_id IS NOT NULL;
CREATE INDEX idx_crm_transactions_entity ON customer360.crm_transactions (entity_type, entity_id) WHERE entity_id IS NOT NULL;
CREATE UNIQUE INDEX ux_crm_transactions_tenant_source ON customer360.crm_transactions (tenant_id, source_system, source_transaction_id) WHERE source_transaction_id IS NOT NULL;
